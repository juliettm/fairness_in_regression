...
class SyntheticAthleteDataset(BaseDataset):
    def __init__(self, att, data_dir=None, random_seed=0, outcome_type='binary'):
        self.outcome_type = outcome_type
        self._outcome_original = 'binary'
        self._name = 'synthetic-athlete'
        self._att = att
        self.preprocess()

    def preprocess(self):

        df_base = pd.read_csv("datasets/synthetic_athlete/preprocessed_synthetic_athlete.csv", sep=',', index_col=0)
        ...
        self._explanatory_variables = ['Age', 'Sex', 'Sport', 'TrainingTime', 'SleepHours', 'Diet']
